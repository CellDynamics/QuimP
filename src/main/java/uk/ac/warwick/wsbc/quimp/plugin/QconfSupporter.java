...
public abstract class QconfSupporter {
  ...
  public QconfSupporter(Path path) {
    try {
      ...
      if (path == null) { // no file provided, ask user
        OpenDialog od =
                new OpenDialog("Open paramater file " + FileExtensions.newConfigFileExt + ")...",
                        OpenDialog.getLastDirectory(), FileExtensions.newConfigFileExt);
  ...
  public ImagePlus getImage() throws QuimpException {
    ...
    ImagePlus im = IJ.openImage(
            ((QParamsQconf) qp).getLoadedDataContainer().getBOAState().boap.getOrgFile().getPath());
  ...
  public boolean validateQconf() throws QuimpException {
    ...
    if (((QParamsQconf) qp).getLoadedDataContainer().QState == null) {
      throw new QuimpException("Q Analysis data not found in QCONF file. Run Q Analysis first.");
    }
    return true;
  }
