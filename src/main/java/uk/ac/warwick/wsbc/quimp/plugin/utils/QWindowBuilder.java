    ...
    for (Map.Entry<String, String> e : def.entrySet()) {
      ...
      switch (uiparams[UITYPE].toLowerCase()) {
        case "spinner": // by default all spinners are double
          ...
          SpinnerNumberModel model = new SpinnerNumberModel(Double.parseDouble(uiparams[S_DEFAULT]), // val
          ...
          ui.put(key + "label", new Label(WordUtils.capitalize(WordUtils.capitalize(key)))); // add
                                                                                             // description
          break;
        case "choice":
