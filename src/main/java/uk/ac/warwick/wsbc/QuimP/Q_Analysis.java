 ...
 * 
 * In principle this object loads and process name_X.paQP file generated by ECMM. \i X in this case
 * means number of cell outline. The same result can be achieved by loading QCONF file that contains
 * all outlines for given case. This class is designed to process one outline in one time. Thus most
 * of methods operate on current status private fields (such as \a qp, \a oH). The main analysis
 * runner method is \a run(), whereas support for both input formats is covered by \a runFromPAQP()
 * and \a runFromQCONF() (similarly to @ref uk.ac.warwick.wsbc.QuimP.ECMM_Mapping "ECMM_Mapping") 
 *  
 * @author rtyson
...
public class Q_Analysis {
     ...
     * @waring uk.ac.warwick.wsbc.QuimP.Q_Analysis.run() updates also ECMMstate by modifying
...
class STmap implements IQuimpSerialize {
     ...
     */
    private int res; //!< resolution of maps
    private int T; //!< number of outlines
     ...
     * @warning Make a copy of serializable fields only
     */
    public STmap(final STmap src) {
        ...
        this.migMap = QuimPArrayUtils.copy2darray(src.migMap, null);
