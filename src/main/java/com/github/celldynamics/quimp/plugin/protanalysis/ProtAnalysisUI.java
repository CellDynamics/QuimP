...
class ProtAnalysisUI implements ActionListener {
  ...
  // points selected by user for current frame, cleared on each slice shift
  ...
  class CustomStackWindow extends StackWindow {
    ...
    public void buildWindow() {
      ...
      selectPoints.setBorder(BorderFactory.createTitledBorder("Select points"));
      JButton bnClear = new JButton();
      bnClear.setAction(new ActionClearPoints("Clear", "Clear points", ProtAnalysisUI.this));
      selectPoints.add(bnClear);

      left.add(selectPoints);
  ...
  class CustomCanvas extends ImageCanvas {
    ...
    public void mousePressed(MouseEvent e) {
      ...
      if (SwingUtilities.isLeftMouseButton(e)
        ...
        if (pc != null) {
          if (selected.add(pc) == false) { // already exists
            selected.remove(pc); // so remove
          }
        }
      } else {
