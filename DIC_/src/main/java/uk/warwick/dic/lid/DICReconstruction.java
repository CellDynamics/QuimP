 ...
 * 
 * This algorithm uses corrected line integration that runs in both directions from current point of image \f$(x_n,y_n)\f$
 * to \f$r_1\f$ and end \f$r_2\f$. Vector \f$dr\f$ is set to be parallel to \a 0X axis. Final formula for reconstruction
 * of pixel \f$S(x_n,y_n)\f$ is given by:
 * 
 * \f[
 * S(x_n,y_n)=\int_{r_1}^{(x_n,y_n)}\left ( I(r)-I_{i,0}) \right )e^{-\delta\left | r_n-r \right | }dr-\int_{r_2}^{(x_n,y_n)}\left ( I(r)-I_{i,0}) \right )e^{-\delta\left | r-r_n \right | }dr
 * \f]
 * 
 * The algorithm perform the following steps to reconstruct whole image:
 * \li Input is converted to 16bit and stored in private field ExtraImageProcessor srcImageCopyProcessor
 * \li The value of \c shift is added to all pixels of input image. All operations are performed on copy of input \c ImageProcessor or \c ImagePlus.
 * \li Rotate image by \c angle to bring bas-reliefs perpendicular to \a OX axis. Rotated image has different dimensions and it is padded by 0.
 * After this operation 0 pixels are those that belong to background.
 * \li For every line in rotated image position of \b true pixels is calculated. \b True pixels are those that belong to original image excluding background
 * added during rotation. For every line position of first and last \b true pixel is noted in table \c ranges
 * \li Decay factors are pre-calculated and stored in \c decays table. 
 * \li Final reconstruction is performed.
 * 
 * Image for reconstruction is passed during construction of DICReconstruction object. For this object \c ranges and \c decays are
 * evaluated and then user can call reconstructionDicLid() method to get reconstruction. getRanges() method also rotates private object
 * thus rotation in reconstructionDicLid() is not necessary (flagged by DICReconstruction::isRotated). 
 * Different situation happens when the whole stack is reconstructed. To prevent creating new instance of DICReconstruction 
 * for every slice the setIp(ImageProcessor) method is used for connecting new slice. In this case it is assumed that ImageProcessor
 * objects are similar and they have the same geometry. \c ranges are filled only once on DICReconstruction constructing thus images
 * connected by setIp(ImageProcessor) are not rotated. This situation is detected in reconstructionDicLid() by isRotated flag.     
 *  
 * @warning Currently this class supports only 8bit images. It can support also 16bit input but in this case algorithm
 * used for detection of \b true pixels may not work correctly for certain cases - when maximum intensity will be \f$\mathrm{max}(\mathrm{int})-shift\f$
 * @author p.baniukiewicz
 * @date 10 Dec 2015
 * @see Z. Kam, “Microscopic differential interference contrast image processing by line integration (LID) and deconvolution,” Bioimaging, vol. 6, no. 4, pp. 166–176, 1998.
 * @see B. Heise, A. Sonnleitner, and E. P. Klement, “DIC image reconstruction on large cell scans.,” Microsc. Res. Tech., vol. 66, no. 6, pp. 312–320, 2005.
 *
 */
public class DICReconstruction {
	...
	private ImageProcessor srcIp; ///< local reference of ImageProcessor passed to object. Not modifiable.  
	 ...
	 * should have the same architecture as image passed in constructor. Typically this method is used
	 * for passing next slice from stack.
	 * 
	 * @remarks Input \c ip is not modified
	...
	/**
	 * Recalculates true pixels range and new size of image after rotation. Setup private class fields.
	 * 
	 ...
	 * @return Modifies private class fields:
   ...
   /**
	 * Reconstruct DIC image by LID method. This is main method used to reconstruct passed /c ip object.
	 * 
	 ...
	 * will be 16-bit and it will contain saturated pixels
