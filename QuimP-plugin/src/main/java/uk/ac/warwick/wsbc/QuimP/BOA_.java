...
public class BOA_ implements PlugIn {
   ...
   public void run(String arg) {
      ...
      if (ip.getType() != ImagePlus.GRAY8) {
         YesNoCancelDialog ync = new YesNoCancelDialog(window, "Image bit depth", "8-bit Image required. Convert?");
         if (ync.yesPressed()) {
            if (BOAp.singleImage) {
...
class CustomCanvas extends ImageCanvas {
      ...
      public void mousePressed(MouseEvent e) {
         ...
         if (BOAp.doDelete) {
            //BOA_.log("Delete at: ("+offScreenX(e.getX())+","+offScreenY(e.getY())+")");
...
class CustomStackWindow extends StackWindow implements ActionListener, ItemListener, ChangeListener {
      ...
      public void updateSliceSelector() {
         ...
         zSelector.setValue(imp.getCurrentSlice()); // this is delayed in super.updateSliceSelector
                                                    // force it now
   ...
   public void runBoa(int startF, int endF) throws BoaException {
      ...
      try {
         ...
         if (!BOAp.expandSnake) {      // blowup snake ready for contraction (only those not starting at or after the startF)
         ...
         for (frame = startF; frame <= endF; frame++) { // per frame
            ...
            try {
               ...
               //imageGroup.drawContour(nest.getSNAKES(), frame);//draw starting snake
               //if(frame==2) break;
               ...
               for (s = 0; s < nest.size(); s++) { //for each snake
                  ...
                  try {
                     ...
                     BOAp.SEGrunning = false;
                     if (nest.allDead()) {
                        throw new BoaException("All snakes dead: " + be.getMessage(), frame, 1);
                     }
                  }
   ...
   private void tightenSnake(Snake snake) throws Exception {
      ...
      for (i = 0; i < BOAp.max_iterations; i++) {  // iter constrict snake
         ...
         if ((snake.getNODES() / snake.startingNnodes) > BOAp.NMAX) { // if max nodes reached (as % starting) prompt for reset
            if (BOAp.use_previous_snake) {
               //imageGroup.drawContour(snake, frame);
               //imageGroup.updateAndDraw();
               throw new BoaException("Frame " + frame + "-max nodes reached " + snake.getNODES(), frame, 1);
            } else {
   ...
   boolean loadSnakes(){
      ...
      YesNoCancelDialog yncd= new YesNoCancelDialog(IJ.getInstance(), "Load associated snakes?","\tLoad associated snakes?\n");
      if(!yncd.yesPressed()){
         return false;
      }
   ...
   void deleteSegmentation(int x, int y, int frame) {
      ...
      int minIndex = Tool.minArrayIndex(distance);
      //BOA_.log("Debug: closest index " + minIndex + ", id " + nest.getHandler(minIndex).getID());
      if (distance[minIndex] < 10) { // if closest < 10, delete it
         BOA_.log("Deleted snake " + nest.getHandler(minIndex).getID() + " from " + frame + " onwards");
      ...
      if (BOAp.saveSnake) {
         try {
            if (nest.writeSnakes()) {
               ...
               ync = new YesNoCancelDialog(window, "Save Segmentation", "Quit without saving?");
               if (!ync.yesPressed()) {
                  return;
               }
            }
   ...
   public static void main(String[] args) {
      ...
      //ij.ImageJ.main(new String[]{"-ijpath", "/Users/rtyson/Documents/phd/NetBeans/ImageJ/"});
   }
}
...
class ImageGroup {
   ...
   public void updateOverlay(int frame) {
      ...
      for (int i = 0; i < nest.size(); i++) {
         ...
         if (sH.isStoredAt(frame)) { //is there a snake a;t f?
            ...
            //draw centre point
            PointRoi pR = new PointRoi((int) snake.getCentroid().getX(), (int) snake.getCentroid().getY());
            overlay.add(pR);
   ...
   void zoom(ImageCanvas ic, int frame) {
      ...
      r.setBounds(r.x - border, r.y - border, r.width + border * 2, r.height + border * 2); //add border (10 either way)
      ...
      //System.out.println("ic " + icAspect + ". rA: " + r.getWidth() / r.getHeight());
...
class Constrictor {
   ...
   public boolean constrictWrite(Snake snake, ImageProcessor ip) {
      ...
      try {
         PrintWriter pw = new PrintWriter(new FileWriter("/Users/rtyson/Documents/phd/tmp/test/forcesWrite/forces.txt"), true); //auto flush
   ...
   public ExtendedVector2d imageForceOLD(Node n, ImageProcessor ip) {
      ...
      for (i = 0; i <= 1. / a * BOAp.sample_tan; ++i) {
         ...
         for (j = 0; j <= 1. / a * BOAp.sample_norm / 2; ++j) {
            ...
            if (x > 0 && y > 0 && x <= BOAp.WIDTH && y <= BOAp.HEIGHT) //check that
            //pixel is inside frame
            {
      ...
      // if (I_out > BOAp.sample_norm / 2 * BOAp.sample_tan) //check that all I_out pixels are inside the frame
      // {
      ...
      for (i = 0; i <= 1. / a * BOAp.sample_norm; ++i) {
         ...
         for (j = 0; j <= 1. / a * BOAp.sample_tan / 2; ++j) {
            ...
            x = xt - a * j * n.getNormal().getX();
            y = yt - a * j * n.getNormal().getY();
            if (x > 0 && y > 0 && x <= BOAp.WIDTH && y <= BOAp.HEIGHT) //check that pixel is inside frame
            {
               I_outside += ip.getPixel((int) x, (int) y);
               ++I_out;
            }
      ...
      //if contrast is positive compute image force as square root of the contrast
   ...
   public ExtendedVector2d imageForce(Node n, ImageProcessor ip) {
      ...
      double xt, yt;		//co-ordinates of the tangent
      int I_inside = 0;
      int I_outside = 0;      //Intensity of neighbourhood of a node (insde/outside of the chain)
      int I_in = 0, I_out = 0;	//number of pixels in the neighbourhood of a node
      ...
      for (i = 0; i <= 1. / a * BOAp.sample_tan; i++) {
         ...
         for (j = 0; j <= 1. / a * BOAp.sample_norm / 2; ++j) {
            ...
            //System.out.println("outer pixel I = " + ip.getPixel((int) x, (int) y));
            //}
   ...
   public void loosen(Nest nest, int frame) throws Exception {
      ...
      double[][] prox = new double[N][N];  //dist beween snake centroids, triangular
...
class Nest {
   ...
   public void analyse(ImagePlus oi) {
      ...
      while (sHitr.hasNext()) {
         ...
         File pFile = new File(BOAp.outFile.getParent(), BOAp.fileName + "_" + sH.getID() + ".paQP");
         QParams newQp = new QParams(pFile);
         newQp.readParams();
         outputH = new OutlineHandler(newQp);


         File statsFile = new File(BOAp.outFile.getParent() + File.separator + BOAp.fileName + "_" + sH.getID() + ".stQP.csv");
...
class SnakeHandler {
   ...
   public void storeCurrentSnake(int frame) throws Exception {
      ...
      snakes[frame - startFrame] = new Snake(head, liveSnake.getNODES() + 1, ID); // +1 dummy head
   ...
   public boolean writeSnakes() throws Exception {
      ...
      PrintWriter pw = new PrintWriter(new FileWriter(BOAp.outFile), true); //auto flush
   ...
   private void write(PrintWriter pw, int frame, int NODES, Node n) {
      ...
      do {
         pw.print("\n" + IJ.d2s(n.position, 6)
                 + "\t" + IJ.d2s(n.getX(), 2) + "\t" + IJ.d2s(n.getY(), 2)
                 + "\t0\t0\t0"
                 + "\t-2\t-2\t-2\t-2\t-2\t-2\t-2\t-2\t-2"); // fluo values (x,y, itensity)
         n = n.getNext();
      } while (!n.isHead());
   ...
   public int snakeReader(File inFile) throws Exception {
      ...
      try {
         ...
         while ((thisLine = br.readLine()) != null) { // while loop begins here
            ...
            snakes[s] = new Snake(head, N + 1, ID); //dont forget the head node
            s++;
         } // end while
      } catch (IOException e) {
   ...
   void resetForFrame(int f) {
      try {
         if (BOAp.use_previous_snake) {
            liveSnake = new Snake((PolygonRoi) this.getStoredSnake(f - 1).asFloatRoi(), ID); // set to last segmentation ready for blowup
         } else {
            liveSnake = new Snake(roi, ID, false);
         }
      } catch (Exception e) {
      ...
      //for (int i = f - startFrame; i < snakes.length; i++) { // keep other snakes?
      //snakes[i] = null;
      //}
   }
...
class Snake {
   ...
   //private double detO;              // determinant of orientation (calc weather concave or convex)
   ...
   public void printSnake() {
      ...
      do {
         ...
         System.out.println(i + " Node " + n.getTrackNum() + ", x:" + x + ", y:" + y + ", vel: " + n.getVel().length());
         n = n.getNext();
         i++;
      } while (!n.isHead());
      if (i != NODES) {
   ...
   public void cutLoops() {
      ...
      do {
         ...
         interval = (NODES > MAXINTERVAL + 3) ? MAXINTERVAL : (NODES - 3); //always leave 3 nodes, at least
   ...
   public void cutIntersects() {
      ...
      do {
         cutHead = (nA.getNext().isHead()) ? true : false;
         nB = nA.getNext().getNext();// don't check next edge as they can't cross, but do touch
         interval = (NODES > 6) ? NODES / 2 : 2; //always leave 3 nodes, at least
         ...
         for (int i = 2; i < interval; i++) {
            ...
            if (state == 1) {
               //System.out.println("CutIntersect: cut out an intersect: x0: " + 
                        //nA.getX() + ", y0:" + nA.getY()+ ", x1 :" +nA.getNext().getX()+ ", y1: " +nA.getNext().getY() +
                        //", x2: "+nB.getX()+ ", y2: " + nB.getY()+ ", x3: " +nB.getNext().getX()+ ", y3: " + nB.getNext().getY());
   ...
   public void cutLoopsOLD() {
      ...
      do {
         ...
         for (i = 1; i <= INTERVALL; ++i) {
            ...
            if (node1.getTrackNum() == right2.getTrackNum()) { //dont go past node1
               break;
            } else if (((diffX * node2.getY() - diffY * node2.getX())
                    < (diffX * node1.getY()
                    - diffY * node1.getX()) ^ (diffX * right2.getY()
                    - diffY * right2.getX())
                    < (diffX * node1.getY()
                    - diffY * node1.getX())) & ((diffXp * node1.getY()
                    - diffYp * node1.getX())
                    < (diffXp * node2.getY()
                    - diffYp * node2.getX()) ^ (diffXp * right1.getY()
                    - diffYp * right1.getX())
                    < (diffXp * node2.getY()
                    - diffYp * node2.getX()))) {
               ...
               //IJ.log("Cut Loop! cut from node1 " + node1index + " to right2 " + right2index + " interval " + i);
   ...
   public void correctDistanceOLD() throws Exception {
      ...
      avg_dist = 0.5 * (BOAp.getMin_dist() + BOAp.getMax_dist()); //compute average distance
      ...
      do {
         ...
         if (Di > 2. * avg_dist) { //distance greater than DistMax: add in node
         ...
         } else if (Di < BOAp.getMin_dist() && NODES >= 4) { // Minimum Nodes is 3
...
//    colour = QColor.lightColor();
   ...
   public void updateNormale() {
      ...
      double localO = (xb*yc + xa*yb + ya*xc) - (ya*xb + yb*xc + xa*yc); //determinant of orientation
      if(localO==0){
      IJ.log("orientation is flat!");
      normal.setX(-tan.getY());
      normal.setY(tan.getX());
      }else if(localO*detO > 0){
      normal.setX(-tan.getY());
      normal.setY(tan.getX());
      }else{
      normal.setX(tan.getY());
...
class BOAp {
   ...
   static public void setup(ImagePlus ip) {
      ...
      if (imageScale == 0) {
         imageScale = 1;
         scaleAdjusted = true;
         //BOA_.log("Warning. Scale was 1 pixel == 0 \u00B5m. Using 1 \u00B5m instead"
   ...
   static public void writeParams(int sID, int startF, int endF) {
      if (saveSnake) {
         ...
         File statsFile = new File(BOAp.outFile.getParent() + File.separator + BOAp.fileName + "_" + sID + ".stQP.csv");
