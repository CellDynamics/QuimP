...
public class ANA_ implements PlugInFilter, DialogListener {
   ...
   public int setup(String arg, ImagePlus imp) {
      ...
      /*
      IJ.log("Warning: Image was reduced to 8-bit");
   ...
   public void run(ImageProcessor Ip) {
      ...
      /*//angle test
      Vect2d v1 = new Vect2d(-1,0);
      Vect2d v2 = new Vect2d(0.5,1);
      v2.makeUnit();
      double angle = Vect2d.angle(v1, v2);
      ...
      try {
         do {
            ...
            qp.readParams();// will NOT know startFrame or endFrame until outlines read if old format!!
            ...
            qp.fluTiffs[ANAp.channel] = new File(orgIpl.getOriginalFileInfo().directory, orgIpl.getOriginalFileInfo().fileName);
            ...
            //----Write temp files-------
            //File tempFile = new File(ANAp.OUTFILE.getAbsolutePath() + ".tempANA.txt");
            //outputH.writeOutlines(tempFile);
            //File tempStats = new File(ANAp.STATSFILE.getAbsolutePath() + ".tempStats.csv");
   ...
   private void Ana() {
      ...
      //System.out.println("start frame: " + ANAp.startFrame + ". end frame: ," +ANAp.endFrame);
      ...
      for (int f = oH.getStartFrame(); f <= oH.getEndFrame(); f++) { // change i to frames
         ...
         if (ANAp.sampleAtSame && ANAp.useLocFromCh != -1) {
            ...
            // some nodes may fail to migrate properly so need to check tracknumbers match
            Vert v = o1.getHead();
            ...
            int vStart;
            do {
               //v.fluores[ANAp.channel] = v2.fluores[0].copy(); // measurments are stored in channel 0 by ECMM
               ...
               do {
                  v = v.getNext();
                  v.setFluoresChannel((int) Math.round(v.getX()), (int) Math.round(v.getY()), -1, ANAp.channel); // map fail if -1. fix by interpolation
            ...
            //o1.getHead().getNext().fluores[ANAp.channel].intensity = -1; // test correction
         ...
         if(f == oH.getEndFrame()) drawSamplePointsFloat(o1);

         //System.out.println("Nb nodes: "+o1.getVerts() + ", overlay size: "+overlay.size());
         orgIpl.draw();
   ...
   private void shrink(Outline o) {
      ...
      //System.out.println("steps: " + steps + ", step size: " + ANAp.stepRes);
      Vert n;
      int j;
      int max = 10000;
      for (j = 0; j < steps; j++) {
            ...
            do {
               ...
               if (dis < ANAp.freezeTh) {
                  ...
                  if ( angle < ANAp.angleTh  && angle > -ANAp.angleTh ) {
                     ...
                     //System.out.println("Dis: "+dis+", angle:"+angle+", vNum:"+v.getTrackNum()+", vTNum:"+ vT.getTrackNum());
                     //System.out.println("v.x: " + v.getX() + " , v.y: " + v.getY());
                     //System.out.println("vt.x: "+vT.getX()+", vt.y:"+vT.getY()+", vt.n.x:"+vT.getNext().getX()+", vt.n.y:"+vT.getNext().getY());
                     //System.out.println("c.x: " + closest.getX()+", c.y: "+ closest.getY());
                     //break;
                  }
...
//   }
   ...
   private void setFluoStats(Polygon outerPoly, Polygon innerPoly, int f) {
      ...
      fluoStats[store].channels[ANAp.channel].totalFluor = is.mean * is.area;
      fluoStats[store].channels[ANAp.channel].meanFluor = is.mean; //fluoStats[store].channels[ANAp.channel].totalFluor / fluoStats[store].area;
      ...
      fluoStats[store].channels[ANAp.channel].innerArea = Tool.areaToScale(is.area, ANAp.scale);
      fluoStats[store].channels[ANAp.channel].totalInnerFluor = is.mean * is.area;
      fluoStats[store].channels[ANAp.channel].meanInnerFluor = is.mean; //fluoStats[store].channels[ANAp.channel].totalInnerFluor / fluoStats[store].channels[ANAp.channel].innerArea;


      fluoStats[store].channels[ANAp.channel].cortexArea = fluoStats[store].area - fluoStats[store].channels[ANAp.channel].innerArea; //scaled
      fluoStats[store].channels[ANAp.channel].totalCorFluo = fluoStats[store].channels[ANAp.channel].totalFluor - fluoStats[store].channels[ANAp.channel].totalInnerFluor;
      fluoStats[store].channels[ANAp.channel].meanCorFluo = fluoStats[store].channels[ANAp.channel].totalCorFluo / (outerAreaRaw - is.area); //not scaled
   ...
   private void normalise2Interior(Outline o, int f) {
      ...
      do {
         ...
         v.fluores[ANAp.channel].intensity = v.fluores[ANAp.channel].intensity / fluoStats[store].channels[ANAp.channel].meanInnerFluor;
         //System.out.print(" to " + v.fluores[ANAp.channel].intensity);
         //System.out.print(", by mean fluo: " + fluoStats[store].channels[ANAp.channel].meanInnerFluor + "\n");
         v = v.getNext();
      } while (!v.isHead());
   ...
   private void drawOutlineAsOverlay(Outline o, Color c) {
      Roi r = o.asFloatRoi();
      if(r.subPixelResolution()) System.out.println("is sub pixel");
      else System.out.println("is not sub pixel");
   ...
   private void investigateChannels(Outline o) {
      ...
      if(Tool.sumArray(ANAp.presentData)==3) firstEmptyCh = 0;
   ...
   private void interpolateFailures(Outline o) {
      ...
      do {
         ...
         if (v.fluores[ANAp.channel].intensity == -1) {
            ...
            //IJ.log("n.prev: "+last.fluores[ANAp.channel].intensity +", n.next:" + nex.fluores[ANAp.channel].intensity + ", new inten: " + v.fluores[ANAp.channel].intensity);
         }
...
/*   private void drawSamplePoints(Outline o) {
      ...
      Vert v = o.getHead();
      do {
         x = (int) v.fluores[ANAp.channel].x;
         y = (int) v.fluores[ANAp.channel].y;
         pr = new PointRoi(x, y);
         overlay.add(pr);
         v = v.getNext();
      } while (!v.isHead());
...
class FluoStats {
   ...
   public static void write(FluoStats[] s, File OUTFILE) {
      try {
         ...
         pw.print("# Scale: " + ANAp.scale + " micro meter per pixel | Frame interval: " + ANAp.frameInterval + " sec\n");
         ...
         for (int i = 0; i < s.length; i++) {
            pw.print("\n" + s[i].frame + "," + IJ.d2s(s[i].centroid.getX(), 2) + "," + IJ.d2s(s[i].centroid.getY(), 2)
   ...
   private static void writeFluo(FluoStats[] s, PrintWriter pw, int c) throws Exception {
      pw.print("\n#\n# Channel " + (c + 1) + ";Frame, Total Fluo.,Mean Fluo.,Cortex Width, Cyto. Area,Total Cyto. Fluo., Mean Cyto. Fluo.,"
              + "Cortex Area,Total Cortex Fluo., Mean Cortex Fluo., %age Cortex Fluo.");
      for (int i = 0; i < s.length; i++) {
         pw.print("\n" + s[i].frame + "," + IJ.d2s(s[i].channels[c].totalFluor) + "," + IJ.d2s(s[i].channels[c].meanFluor) + "," + IJ.d2s(s[i].channels[c].cortexWidth));
         pw.print("," + IJ.d2s(s[i].channels[c].innerArea) + "," + IJ.d2s(s[i].channels[c].totalInnerFluor) + "," + IJ.d2s(s[i].channels[c].meanInnerFluor));
         pw.print("," + IJ.d2s(s[i].channels[c].cortexArea) + "," + IJ.d2s(s[i].channels[c].totalCorFluo) + "," + IJ.d2s(s[i].channels[c].meanCorFluo) + "," + IJ.d2s(s[i].channels[c].percCortexFluo));
      }
   }
...
class ANAp {
   ...
   static void setup(QParams qp) {
      ...
      STATSFILE = new File(qp.statsQP.getAbsolutePath()); // output file (.stQP.csv) file
      scale = qp.imageScale;
