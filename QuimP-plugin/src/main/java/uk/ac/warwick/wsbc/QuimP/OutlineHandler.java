...
public class OutlineHandler {
   ...
   public OutlineHandler(QParams params) {
      ...
      //System.out.println("start frame: " + startFrame + ", endframe: " + endFrame);
   ...
   private boolean readOutlines(File f) {
      ...
      try {
         ...
         while ((thisLine = br.readLine()) != null) { // while loop begins here
            ...
            //System.out.println("OH.234.VERTS NOT CORREECT. VERTS: "+outlines[s].getVerts()+", Count: " + c);
            //}
   ...
   private void findStatLimits() {
      ...
      for (int i = 0; i < outlines.length; i++) {
         ...
         do {
            ...
            if(n.distance < migLimits[0]) migLimits[0] = n.distance;
            if(n.distance > migLimits[1]) migLimits[1] = n.distance;

            //if(n.convexity < convLimits[0]) convLimits[0] = n.convexity;
            //if(n.convexity > convLimits[1]) convLimits[1] = n.convexity;

            for (int j = 0; j < n.fluores.length; j++) {
               if(n.fluores[j].intensity < fluLims[j][0]) fluLims[j][0] = n.fluores[j].intensity;
               if(n.fluores[j].intensity > fluLims[j][1]) fluLims[j][1] = n.fluores[j].intensity;
            }
   ...
   public void writeOutlines(File outFile, boolean ECMMrun) {
      try {
         ...
         //pw.write("\n#Frame "+(i+1));
         //if (outlines[i] == null) {
         //System.out.println("DEBUG writeOutlines: frame " + i + " is NULL, not writting");
         //continue;
         //}

         //OutlineHandler.write(pw, i + 1, outlines[i].getVerts(), outlines[i].getHead());
         //}
         pw.close();
      } catch (Exception e) {
   ...
   static private void write(PrintWriter pw, int VERTS, Vert v) {
      ...
      do {
         pw.print("\n" + IJ.d2s(v.coord, 6)
                 + "\t" + IJ.d2s(v.getX(), 2) + "\t" + IJ.d2s(v.getY(), 2)
                 + "\t" + IJ.d2s(v.fCoord, 6) + "\t" + IJ.d2s(v.gCoord, 6)
                 + "\t" + IJ.d2s(v.distance, 6)
                 + "\t" + IJ.d2s(v.fluores[0].intensity, 6) + "\t" + IJ.d2s(v.fluores[0].x, 0) + "\t" + IJ.d2s(v.fluores[0].y, 0)
                 + "\t" + IJ.d2s(v.fluores[1].intensity, 6) + "\t" + IJ.d2s(v.fluores[1].x, 0) + "\t" + IJ.d2s(v.fluores[1].y, 0)
                 + "\t" + IJ.d2s(v.fluores[2].intensity, 6) + "\t" + IJ.d2s(v.fluores[2].x, 0) + "\t" + IJ.d2s(v.fluores[2].y, 0));
         v = v.getNext();
      } while (!v.isHead());
   }
}
