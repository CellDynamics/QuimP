...
/**
 * Just a class for colors, that does not use stupid floats
 * represented as RGB in the range 0->1
 *
 * @author tyson
 */
public class QColor {
    ...
    public static int bwScale(double value, int bits, double max, double min){
        if(value > max || value < min){
            //System.out.println("Warning! bwScale value not in range: "+value+" (min:"+ min +", max:" + max+")");
            Exception e = new Exception();
            e.printStackTrace();
            System.exit(1);
            return 0;
        ...
        //if(value==255)System.out.println("value: " + value+", min:" + min + ", max:" + max+", bits:" + bits+",temp: "+temp+", answ: " + answ);
