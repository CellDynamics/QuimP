...
# This image is now stored in Quimp.wiki repo
...
if [ $UPLOAD -eq 1 ]; then
	rsync -lrtz  -e "ssh -i ~/.ssh/quimp-backend" --delete --stats *.png html/ admin@pilip.lnx.warwick.ac.uk:/data/www/html/restricted/Doxygen_doc
fi
