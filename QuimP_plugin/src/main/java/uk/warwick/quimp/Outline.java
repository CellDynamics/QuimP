...
public final class Outline implements Cloneable {
   ...
   public void plotOutline(String title, boolean per) {
      double[] xArr = new double[VERTS]; // arrays to hold x and y co-ords (duplicate last to join)
   ...
   public boolean cutSelfIntersectsOLD() {
      ...
      do {
         ...
         interval = (VERTS > 6) ? VERTS / 2 : 3; //always leave 3 nodes, at least. Check half way around
         ...
         for (int i = 0; i < interval; i++) {
            ...
            if (intersect != null) {
               ...
               newN.setX(intersect.getX());
               newN.setY(intersect.getY());
   ...
   public boolean cutSelfIntersects() {
      ...
      do {
         ...
         nB = nA.getNext().getNext(); // don't check the next one along! they touch, not overlap
         interval = (VERTS > 6) ? VERTS / 2 : 2; //always leave 3 nodes, at least. Check half way around
         ...
         for (int i = 2; i < interval; i++) {
            ...
            if (state == -1) {
               System.out.println("\nLines parallel");
               System.out.println("close all;plot([" + nA.getX() + "," + nA.getNext().getX() + "],[" + nA.getY() + "," + nA.getNext().getY() + "],'-ob');"); // matlab output
               System.out.println("hold on; plot([" + nB.getX() + "," + nB.getNext().getX() + "],[" + nB.getY() + "," + nB.getNext().getY() + "],'-or');");
            ...
            if (state == 1) {
               //c= countVERTS();
               //if(c !=VERTS){
                  //System.out.println("A -VERTS NOT CORREECT. VERTS: "+VERTS+", Count: " + c);
               //}else{
                  //System.out.println("A -verts correct. VERTS: "+VERTS+", Count: " + c);
               //}
               ...
               VERTS -= (i); 
               //if(VERTS<3) System.out.println("OUTLINE 596_VERTS LESS than 3");

               //c = countVERTS();
               //if(c !=VERTS){
                  //System.out.println("B - VERTS NOT CORREECT. VERTS: "+VERTS+", Count: " + c+ ", i: "+i);
                  //VERTS = c;
               //}else{
      ...
      do {
         ...
         if (v.gCoord >= 1 || v.gCoord < 0 || v.fCoord >= 1 || v.fCoord < 0 || v.coord >= 1 || v.coord < 0) {
            System.out.println("Outline587: Errors in tracking Coordinates\n\t"
                    + "coord=" + v.coord + ", gCoord= " + v.gCoord + ", fCoord = " + v.fCoord);
            return true;
         }
